...
- Started the authoritative local-state read from a tiny classic pre-module bootstrap so `storage.local` IPC can overlap the large ES-module graph parse/evaluation. The main module consumes the same frozen four-key authoritative read; no alternate persistence path, new permission or trust shortcut was introduced.
- Changed hidden closed-folder artwork warming from one large post-startup batch into bounded chunks with explicit yields between slices. Folder records remain complete from startup, the first four visible mosaic pixels still hydrate first, and opening a folder hydrates its missing artwork immediately. Added mutation-generation guards so a late async folder-artwork result cannot overwrite a newer structural edit.
- Extended local-only startup diagnostics with approximate first-launcher/Perceived Complete Paint timing and a bounded long-task observation window. The data remains ephemeral in the current New Tab context only: no persistence, messaging, network reporting or telemetry.
- Fixed the separate Light/Dark wallpaper Settings regression where toggling or changing the active theme wallpaper could leave the modal blank/white even though the setting saved. While Settings is open, wallpaper and darkness changes now paint only the isolated preview layer—including its dim overlay—and leave every real root/page paint value frozen until the deferred post-close commit.
- Made Frequently Visited **Show** and **Count** profile preferences synchronize through MosaicSync settings records while keeping the actual sites/history, hidden-site list and optional Top Sites permission device-local. A receiving computer with synchronized Show=ON keeps the toggle ON; if its local permission is missing it exposes the existing Grant permission action, and the browser permission request remains user-gesture-only. Legacy migration avoids letting a default OFF/5 device race against and overwrite another computer's meaningful legacy ON/non-default preference.
- Increased the shortcut hover enlargement from the intentionally tiny 1.8% effect to a more noticeable 4.5% paint-only scale with slightly stronger brightness. Grid geometry/density and layout remain unchanged.
- Added dedicated regression coverage for secondary-only/critical CSS partitioning, pre-module local-state key contract, bounded folder-artwork hydration, folder-open mutation races, synchronized Frequently Visited preference semantics, local-only paint/long-task diagnostics, Settings-open Light/Dark wallpaper paint isolation and the stronger no-layout-shift hover.
- No new permissions or host permissions, no Sync shortcut/profile/container schema change, no conflict/tombstone change, no image/SVG validation reduction, no navigation-safety change, no CSP relaxation, no telemetry, remote code or artwork-quality reduction. Version identity is exactly `1.27.8.4` across Firefox, Chrome, runtime/UI metadata, tests, documentation and build output.

## 1.27.8.3

- Made the second New Tab performance pass target the actual first-visible/first-interactive critical path on older CPUs. MosaicSync now starts the authoritative `storage.local` read as soon as the New Tab module begins, overlapping extension-storage IPC with localization and secondary UI setup instead of waiting until the bottom of startup.
- Split New Tab styling into a ~43 KB critical launcher stylesheet and the existing complete stylesheet. Only the critical grid/Space/background/Frequently Visited/sync-shell rules block the first frame; an external CSP-safe bootstrap loads the full stylesheet on the next animation frame. No inline event handlers, CSP relaxation, visual feature removal or remote resources were introduced.
- Extended bootstrap-grid reuse to cold authoritative startup. When the synchronous bootstrap exactly matches current active-Space revisions, grid geometry/order, shortcut identity/title/navigation targets and the first four folder mosaic child identities, MosaicSync upgrades that already-painted DOM in place and attaches the normal interaction/drag/edit behavior instead of destroying it with a second `grid.replaceChildren()`. Any mismatch falls back to the established full renderer.
- Made closed-folder artwork hydration genuinely visible-first while keeping all folder records authoritative. Cold startup reads artwork only for the four children that can appear in each closed mosaic; children beyond four retain their URLs, IDs, positions and content-addressed asset references and are hydrated in one deferred batch after perceived completion or immediately when the folder is opened.
- Removed another read-only startup projection by reusing a clone of the exact compact state already read from `storage.local` as the optimistic concurrency baseline. Existing writes still canonicalize/validate their baseline inside the write transaction, and legacy/migration startup still builds a full canonical baseline explicitly.
- Added local-only startup phase instrumentation for critical-CSS readiness, boot-grid construction, early local storage, authoritative state readiness, cold boot-grid adoption and deferred folder artwork. Timings live only in the current New Tab page context and are never persisted, transmitted or reported as telemetry.
- Strengthened cold-grid adoption safety with exact shortcut URL/label checks and explicit first-four folder-child IDs in the disposable bootstrap DOM. Added dedicated regressions for visible-only folder pixel reads, deferred completion, compact-baseline reuse, early storage overlap, critical/secondary CSS delivery, strict DOM-adoption fallback and non-persistent diagnostics in Firefox and Chrome.
- No new permissions or host permissions, no Sync/profile/state schema change, no conflict/tombstone change, no image/SVG validation reduction, no navigation-safety change, no CSP relaxation, no telemetry, remote code or artwork-quality reduction. Version identity is exactly `1.27.8.3` across Firefox, Chrome, runtime/UI metadata, tests, documentation and build output.

## 1.27.8.2

- Optimized New Tab startup for older/slower CPUs without changing MosaicSync behavior or image quality. Matching 48×48 bootstrap artwork now stays visible while the authoritative full-resolution image decodes, eliminating the preview/fallback/full-artwork handoff flash that was visible on slower computers. Preview reuse is strictly gated by the current content-addressed artwork identity so changed or cleared icons cannot display stale pixels.
- Added asynchronous decode hints to bootstrap, shortcut, folder and Frequently Visited artwork. Full images replace their tiny matching preview only after the browser reports them decodable; decode failure keeps the valid preview or falls back through the existing safe artwork path.
- Reused already-validated local-asset identities across startup normalization and write-baseline construction. `storage.local` still performs the full image-data validation/hash trust-boundary check, but the exact same bytes are no longer re-hashed multiple times immediately afterward. The permanent stress benchmark shows the 200-artwork normalization path dropping from roughly 100 ms to 30 ms and baseline construction from roughly 104 ms to 35 ms on the build host when the validated memo is reused.
- Removed heavyweight custom wallpaper bytes from the first authoritative active-Space asset batch. MosaicSync keeps the existing tiny synchronous wallpaper preview in place, paints shortcut artwork first, then hydrates exactly the referenced full wallpaper asset on the next frame and upgrades the background without rebuilding the grid. The authoritative content-addressed wallpaper reference remains present throughout.
- Reduced synchronous render-manifest and preview-generation work for closed folders to the first four children, matching the maximum visible folder mosaic cells. Hidden folder contents remain fully authoritative and unchanged; they simply no longer consume first-frame manifest/preview budget.
- Added focused startup-performance regression coverage for wallpaper deferral, single-asset wallpaper upgrade, validated asset-memo propagation, four-child folder manifest projection, identity-gated bootstrap/full-artwork continuity and asynchronous decode hints in Firefox and Chrome. Existing Sync/security/concurrency behavior remains unchanged.
- No new permissions, host permissions, CSP relaxation, telemetry, remote code, Sync/profile/state schema change or visual-quality reduction. Version identity is exactly `1.27.8.2` across Firefox, Chrome, runtime/UI metadata, tests, documentation and build output.
...
- Retained the immediately previous complete profile generation. If Firefox exposes a new snapshot root before all of its chunks, MosaicSync can continue from the prior verified generation instead of exposing a half-restored profile.
- Fixed fresh-profile bootstrap so Personal alone can never finalize Sync. MosaicSync now requires either a verified complete Personal+Work profile generation or independently valid Personal and Work compatibility ledgers. Missing/partial Work stays in the waiting state; an intentionally empty Work Space remains valid through an explicit zero-record dataset/settings snapshot.
- Preserved edits made while a fresh profile is waiting for Sync. Once the complete incoming profile arrives, MosaicSync merges pending local records with it using the existing deterministic record clocks, then publishes only the resulting delta; unrelated remote shortcuts are not replaced by the temporary local view.
- Added automatic Work self-healing. A complete profile snapshot can supply the missing baseline for a torn Work compatibility ledger, merge any newer visible partial records by the existing conflict rules, and recommit a coherent Work dataset marker last. A trusted upgraded 1.27.7 device can bootstrap this safety generation from its already-applied local Work copy; a half-restored device with no applied Work revision is deliberately not allowed to publish an empty Work snapshot.
- Made Sync readiness profile-level. `ready` is no longer valid when Work has not been applied/verified, closing the failure mode where a fresh profile could show Personal while Work remained empty and `lastAppliedWorkSyncRevision` was blank.
- Added a restrained Firefox-style shortcut hover affordance: mouse-hovered tiles grow by 1.8% and brighten slightly via paint-only transforms/filters, with no grid reflow, density change or layout shift. Reduced-motion behavior continues to disable transitions.
- Added focused regression coverage for complete-profile publication, previous-generation fallback wiring, fresh bootstrap completeness, pending-local-edit merge behavior, torn Work repair, intentional no-half-ready semantics and the hover geometry boundary. No new permissions, host permissions, CSP relaxation, telemetry or remote code were added. The synchronized shortcut/settings record schema remains version 10; local Sync bookkeeping advances to meta schema 12. Version identity is exactly `1.27.8` across Firefox, Chrome, runtime/UI metadata, build output and source documentation.
