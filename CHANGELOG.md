...
- Real-hardware testing of internal 1.27.8.7 supplied the decisive Firefox pill diagnosis: startup became clean, but the same white rounded artifact appeared exactly once when the MosaicSync logo first triggered the deferred stylesheet, then never again after that sheet was present. Public 1.27.8.8 removes the logo dependency entirely and keeps the mascot/brand animation critical-only.
