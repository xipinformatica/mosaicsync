...
- Added a focused New Tab critical-path performance architecture for older/slower CPUs: a launcher-only blocking stylesheet, pre-module authoritative `storage.local` I/O, strict in-place adoption of an exactly matching bootstrap grid, visible-first closed-folder artwork hydration, bounded/yielding hidden-artwork warming, and local-only paint/long-task diagnostics.
- Kept Light/Dark wallpaper changes isolated on the preview layer while Settings is open, synchronized Frequently Visited Show/Count intent while keeping actual Top Sites/history/hidden-site data and optional permission device-local, and strengthened the Firefox-style shortcut hover to a restrained paint-only `scale(1.045)` / `brightness(1.065)` with no grid layout shift.
- Tightened first-frame CSS ownership so launcher Settings/artwork selectors stay in `newtab-critical.css`, the vestigial monolithic `newtab.css` remains outside the runtime loading contract, and custom launcher native buttons explicitly suppress platform widget appearance before first paint.
- Removed the old two-animation-frame `newtab-secondary.css` insertion from New Tab startup. `secondary-style-bootstrap.js` is now an idempotent on-demand provider with no startup rAF/timer insertion; the automatically surfaced Website Access prompt is fully critical-styled, while Settings, Bookmarks, shortcut editor, folder/drop UI, Frequently Visited context menu and toast feedback wait for secondary CSS before visibility.
- Real-hardware testing of internal 1.27.8.7 supplied the decisive Firefox pill diagnosis: startup became clean, but the same white rounded artifact appeared exactly once when the MosaicSync logo first triggered the deferred stylesheet, then never again after that sheet was present. Internal 1.27.8.8 removed the logo dependency and the white-pill trigger, but accidentally omitted the critical brand keyframes. Public 1.27.8.9 restores the mascot/brand animation entirely in critical CSS without allowing logo hover to load deferred CSS.
- Made the deferred stylesheet launcher-isolated so its first legitimate activation cannot alter already-painted launcher styling: global `button/input/select` font normalization and `button` color inheritance now live in critical CSS; top-level shortcut/folder color-tag variables and rules are critical-owned; the deferred light-theme `.edit-chip` duplicate and brand keyframes are removed; and the remaining generic Sync-tooltip width rule is scoped to Settings.
- Added regression coverage proving logo hover never requests secondary CSS and rejecting deferred global form resets, launcher edit-chip/color-tag ownership, unscoped launcher Sync-tooltip styling and brand keyframes. The bootstrap/adoption DOM contract remains unchanged so this fix addresses the now-observed stylesheet trigger without introducing a second startup experiment.
- Restored the mascot's `brand-hello-pop` and `brand-easter-wave` keyframes to critical CSS, with positive preservation coverage proving the mascot still activates and negative coverage proving logo hover remains secondary-CSS-free.
- Fixed Light-mode first paint by applying the already-computed effective Light/Dark theme attribute in the blocking appearance hint before launcher critical CSS paints; authoritative storage still reconciles afterward.
- Hardened the open-Settings lifecycle: asynchronous/local-storage/Sync/background state adoption keeps the model current but defers launcher-wide root paint and grid reconstruction until Settings closes, coalescing pending work into one authoritative commit. Direct Settings controls remain live. This addresses the recurring Separate Light/Dark Wallpapers and fresh-device Frequently Visited visual corruption class without restoring the old broad own-write suppression.
- Fixed drag/drop choice localization by refreshing both “Move here / Switch their positions” and folder-action text from locale keys every time the choice UI opens; all 32 locale catalogs are regression-checked.
- Improved automatic/manual favicon candidate suitability without site-specific exceptions: resolution benefit is capped once artwork is tile-ready, conventional/browser favicon provenance is weighted above generic manifest/touch artwork when quality is already sufficient, and strongly non-square assets are penalized. Existing image/SVG/network safety bounds remain unchanged.
