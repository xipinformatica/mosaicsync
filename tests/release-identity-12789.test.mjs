...
const VERSION = "1.27.8.9";

test("1.27.8.9 has one exact release identity across public/runtime surfaces", () => {
  ...
  const releaseNotes = fs.readFileSync("docs/RELEASE-1.27.8.9.md", "utf8");
  const qa = fs.readFileSync("docs/QA-1.27.8.9.md", "utf8");
  ...
  assert.match(constants, /export const VERSION = "1\.27\.8\.9";/);
  assert.match(ffHtml, /MosaicSync · 1\.27\.8\.9/g);
  assert.match(chromeHtml, /MosaicSync · 1\.27\.8\.9/g);
  assert.match(readme, /Current source release: 1\.27\.8\.9/);
  assert.match(devReadme, /1\.27\.8\.9/);
  assert.match(releaseNotes, /^# MosaicSync 1\.27\.8\.9 publication notes/m);
  assert.match(releaseNotes, /## GitHub release title\n\n`MosaicSync 1\.27\.8\.9`/);
  assert.match(qa, /^# MosaicSync 1\.27\.8\.9 QA \/ release-candidate checklist/m);
...
test("1.27.8.9 public changelog keeps internal candidates out of the public release sequence", () => {
  const changelog = fs.readFileSync("CHANGELOG.md", "utf8");
  assert.match(changelog, /^## 1\.27\.8\.9\n/);
