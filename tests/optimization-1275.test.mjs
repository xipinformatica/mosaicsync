...
test("1.27.8 anchors the compact PSL to an independent semantic hash and rule-shape sanity checks", async () => {
...
test("1.27.8 source and compact-runtime PSLs produce identical registrable-domain behavior for exact, private, wildcard, exception and IDN rules", async () => {
  const source = await readFile("src/shared/core/public_suffix_list.dat", "utf8");
  const expected = {
...
test("1.27.8 compact locale helper preserves source key order and hostile/special translation text exactly", async () => {
...
for (const browser of ["firefox", "chrome"]) {
  test(`1.27.8 ${browser} favicon-choice cache is gated by a live Website Access permission read`, async () => {
  ...
  test(`1.27.8 ${browser} redirect favicon shares the existing two-wide manual-discovery batch with conventional fallbacks`, async () => {
    const source = fs.readFileSync(`dist/${browser}/background/background.js`, "utf8");
...
test("1.27.8 New Tab CSS class audit uses class-bearing references instead of arbitrary substrings", async () => {
  for (const browser of ["firefox", "chrome"]) {
...
test("1.27.8.3 size guard detects missing categories and significant individual-file growth while preserving accurate accounting", async () => {
  ...
  for (const browser of ["firefox", "chrome"]) {
    ...
    assert.equal(actual.version, "1.27.8.3");
...
test("1.27.8 JavaScript and Python package-size category classifiers remain identical", () => {
  const paths = [
