...
const VERSION = "1.27.8.8";

test("1.27.8.8 has one exact release identity across public/runtime surfaces", () => {
  ...
  const releaseNotes = fs.readFileSync("docs/RELEASE-1.27.8.8.md", "utf8");
  const qa = fs.readFileSync("docs/QA-1.27.8.8.md", "utf8");
  ...
  assert.match(constants, /export const VERSION = "1\.27\.8\.8";/);
  assert.match(ffHtml, /MosaicSync · 1\.27\.8\.8/g);
  assert.match(chromeHtml, /MosaicSync · 1\.27\.8\.8/g);
  assert.match(readme, /Current source release: 1\.27\.8\.8/);
  assert.match(devReadme, /1\.27\.8\.8/);
  assert.match(releaseNotes, /^# MosaicSync 1\.27\.8\.8 publication notes/m);
  assert.match(releaseNotes, /## GitHub release title\n\n`MosaicSync 1\.27\.8\.8`/);
  assert.match(qa, /^# MosaicSync 1\.27\.8\.8 QA \/ final release checklist/m);
...
test("1.27.8.8 public changelog keeps internal candidates out of the public release sequence", () => {
  const changelog = fs.readFileSync("CHANGELOG.md", "utf8");
  assert.match(changelog, /^## 1\.27\.8\.8\n/);
  for (const unpublished of ["1.27.8", "1.27.8.1", "1.27.8.2", "1.27.8.3", "1.27.8.4", "1.27.8.5", "1.27.8.6", "1.27.8.7", "1.26.13", "1.26.13b", "1.26.14", "1.26.15", "1.26.16", "1.26.17", "1.26.17.1", "1.26.17.2"]) {
    assert.doesNotMatch(changelog, new RegExp(`^## ${unpublished.replaceAll(".", "\\.")}(?:\\s|$)`, "m"));
  }
  const headings = [...changelog.matchAll(/^## ([^\n]+)$/gm)].map(match => match[1]);
  assert.deepEqual(headings.slice(0, 9), [VERSION, "1.27.7", "1.27.6", "1.27.5", "1.27.4", "1.27.3", "1.27.2", "1.27.1", "1.27.0"]);
});
