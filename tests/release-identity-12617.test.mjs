...
test("1.26.17 has one exact release identity across public/runtime surfaces", () => {
  ...
  assert.match(constants, /export const VERSION = "1\.26\.17";/);
  assert.match(ffHtml, /MosaicSync · 1\.26\.17/);
  assert.match(chromeHtml, /MosaicSync · 1\.26\.17/);
  assert.match(readme, /Current source release: 1\.26\.17/);
});

test("1.26.17 public changelog does not invent standalone unpublished releases", () => {
  const changelog = fs.readFileSync("CHANGELOG.md", "utf8");
  assert.match(changelog, /^## 1\.26\.17\n/);
  for (const unpublished of ["1.26.13", "1.26.13b", "1.26.14", "1.26.15", "1.26.16"]) {
