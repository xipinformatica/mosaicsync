...
for (const browser of ["firefox", "chrome"]) {
  ...
  test(`1.27.8.9 ${browser} external Settings-open state commits defer safely while direct Settings controls stay live`, async () => {
    ...
    const context = vm.createContext({
      ...
      state: { shortcuts: [], settings: { columns: 8, rows: 4 } },
    ...
    assert.match(listener, /writeBaseline = createPersistedWriteBaseline\(stateChange\.newValue\);[\s\S]*?applyPendingSettingsDraft\(\);[\s\S]*?refreshSettingsControlsAfterExternalState\([^)]*\);[\s\S]*?reconcileLauncherAfterExternalState\((?:\{[^)]*\})?\);/);
