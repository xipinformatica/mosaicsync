...
for (const browser of ["firefox", "chrome"]) {
  ...
  test(`1.30.18.7 ${browser} Top Sites permission invalidation clears session FV sites but preserves the Show preference`, async () => {
    ...
    try {
      ...
      const next = store[constants.SESSION_RENDER_STATE_KEY];
      assert.equal(next.firstPaint.frequent.enabled, true, "permission removal must not rewrite the synchronized Show preference");
      assert.equal(next.firstPaint.frequent.count, 5);
      assert.deepEqual(next.firstPaint.frequent.sites, []);
