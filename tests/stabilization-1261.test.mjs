...
for (const browser of ["firefox", "chrome"]) {
  test(`1.26.3 ${browser} bookmark color menu stays inside the active modal before entering top layer`, async () => {
    ...
    const match = bookmarksController.match(/function showBookmarkFolderColorMenu\(event, folder\)\s*\{([\s\S]*?)\n  \}\n\n  function createBookmarkFolderButton/);
    ...
    assert.match(bookmarksController, /bookmarkColorMenu\.hidePopover\?\.\(\)/,
