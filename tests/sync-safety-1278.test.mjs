...
test("1.27.8.1 release and local Sync bookkeeping schemas are explicit", () => {
  assert.equal(VERSION, "1.27.8.1");
...
for (const browser of ["firefox", "chrome"]) {
  test(`1.27.8 ${browser} device snapshot is a backward-compatible complete Personal+Work generation`, async () => {
    ...
    assert.match(src, /previousProfile:\s*previousProfileDescriptor\(currentRoot\)/,
      "root must retain a descriptor for the last complete generation");
  ...
  test(`1.27.8 ${browser} shortcut hover is restrained, paint-only and does not change grid geometry`, async () => {
    ...
    assert.match(css, /\.shortcut-card:hover \.tile\s*\{[\s\S]*?transform:\s*scale\(1\.018\);[\s\S]*?filter:\s*brightness\(1\.045\);/);
