...
for (const browser of ["firefox", "chrome"]) {
  test(`1.26.11 ${browser} locale loader exposes only literal dynamic-import targets`, async () => {
    ...
    assert.equal(literalLocaleImports.length, 31, "all 31 non-English locale modules must remain lazy-loaded through literal imports");
  });
}

for (const browser of ["firefox", "chrome"]) {
  test(`1.26.11 ${browser} public Settings label matches the internal/runtime version`, async () => {
    ...
    assert.equal(runtimeVersion, "1.26.11");

    const manifest = JSON.parse(await readFile(`dist/${browser}/manifest.json`, "utf8"));
    assert.equal(manifest.version, runtimeVersion, "manifest version must match internal VERSION");
    if (browser === "chrome") assert.equal(manifest.version_name, runtimeVersion, "Chrome version_name must match internal VERSION");
