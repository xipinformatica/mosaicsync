...
for (const browser of ["firefox", "chrome"]) {
  test(`1.27.8.3 ${browser} locale loader exposes only literal dynamic-import targets`, async () => {
...
for (const browser of ["firefox", "chrome"]) {
  test(`1.27.8.3 ${browser} public Settings label matches the internal/runtime version`, async () => {
    ...
    assert.equal(runtimeVersion, "1.27.8.3");

    const manifest = JSON.parse(await readFile(`dist/${browser}/manifest.json`, "utf8"));
    assert.equal(manifest.version, "1.27.8.3", "technical manifest version must match 1.27.8.3");
