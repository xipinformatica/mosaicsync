...
test("1.33.0.2 compatibility floors and permission budget stay explicit and least-privilege", () => {
  const firefox = json("src/firefox/manifest.json");
  const chrome = json("src/chrome/manifest.json");

  assert.equal(firefox.version, "1.33.0.18");
  assert.equal(chrome.version, "1.33.0.18");
  assert.equal(chrome.version_name, "1.33.0.18");
...
test("1.33.0.2 repository remains dependency-free and maintenance ESM uses built-ins/local modules only", () => {
...
test("1.33.0.2 compatibility and maintenance policy preserve the reviewed freeze boundary", () => {
...
test("1.33.0.2 architecture map links the permanent post-freeze compatibility and maintenance references", () => {
