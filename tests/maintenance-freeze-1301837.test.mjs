...
test("1.31.1 compatibility floors and permission budget stay explicit and least-privilege", () => {
  const firefox = json("src/firefox/manifest.json");
  const chrome = json("src/chrome/manifest.json");

  assert.equal(firefox.version, "1.31.1");
  assert.equal(chrome.version, "1.31.1");
  assert.equal(chrome.version_name, "1.31.1");
...
test("1.31.1 repository remains dependency-free and maintenance ESM uses built-ins/local modules only", () => {
...
test("1.31.1 compatibility and maintenance policy preserve the reviewed freeze boundary", () => {
...
test("1.31.1 architecture map links the permanent post-freeze compatibility and maintenance references", () => {
