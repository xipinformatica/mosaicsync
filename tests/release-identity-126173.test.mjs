...
const VERSION = "1.26.17.3";

test("1.26.17.3 has one exact release identity across public/runtime surfaces", () => {
  ...
  assert.match(constants, /export const VERSION = "1\.26\.17\.3";/);
  assert.match(ffHtml, /MosaicSync · 1\.26\.17\.3/);
  assert.match(chromeHtml, /MosaicSync · 1\.26\.17\.3/);
  assert.match(readme, /Current source release: 1\.26\.17\.3/);
});

test("1.26.17.3 public changelog does not invent standalone unpublished releases", () => {
  const changelog = fs.readFileSync("CHANGELOG.md", "utf8");
  assert.match(changelog, /^## 1\.26\.17\.3\n/);
  ...
  assert.deepEqual(headings.slice(0, 2), [VERSION, "1.26.12"]);
});
