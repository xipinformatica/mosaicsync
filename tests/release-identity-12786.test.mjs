...
const VERSION = "1.27.8.6";

test("1.27.8.6 has one exact release identity across public/runtime surfaces", () => {
  ...
  assert.match(constants, /export const VERSION = "1\.27\.8\.6";/);
  assert.match(ffHtml, /MosaicSync · 1\.27\.8\.6/g);
  assert.match(chromeHtml, /MosaicSync · 1\.27\.8\.6/g);
  assert.match(readme, /Current source release: 1\.27\.8\.6/);
});

test("1.27.8.6 public changelog keeps historical release sequence intact", () => {
  const changelog = fs.readFileSync("CHANGELOG.md", "utf8");
  assert.match(changelog, /^## 1\.27\.8\.6\n/);
  ...
  assert.deepEqual(headings.slice(0, 15), [VERSION, "1.27.8.5", "1.27.8.4", "1.27.8.3", "1.27.8.2", "1.27.8.1", "1.27.8", "1.27.7", "1.27.6", "1.27.5", "1.27.4", "1.27.3", "1.27.2", "1.27.1", "1.27.0"]);
});
