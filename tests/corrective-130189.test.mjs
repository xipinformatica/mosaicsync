...
for (const browser of ["firefox", "chrome"]) {
  test(`1.30.18.9 ${browser} stale FV presentation update cannot downgrade newer shared Space/grid state`, async () => {
    ...
    try {
      ...
      assert.deepEqual(after.shortcuts, before.shortcuts, "FV patch owns only the frequent field");
      assert.equal(after.firstPaint.frequent.sites[0].url, "https://example.com/");
