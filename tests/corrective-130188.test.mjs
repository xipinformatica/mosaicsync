...
for (const browserName of ["firefox", "chrome"]) {
  ...
  test(`1.30.18.8 ${browserName} generic background state writes preserve an active FV permission tombstone`, async () => {
    ...
    try {
      ...
      assert.ok(snap, "background write must still warm a usable session projection");
      assert.deepEqual(snap.firstPaint.frequent, { enabled: true, count: 5, sites: [] },
        "an unrelated state write must not turn permission invalidation back into null/no-op");
      assert.equal(session.data[constants.SESSION_FREQUENTLY_VISITED_SUPPRESSED_KEY], true);
    } finally {
      globalThis.browser = previousBrowser;
    }
  ...
  test(`1.30.18.8 ${browserName} actual identical shared session bytes still avoid a rewrite`, async () => {
    ...
    try {
      ...
      session.data[constants.SESSION_RENDER_STATE_KEY] = storage.createRenderSnapshot(state, { frequentSnapshot: frequent });
