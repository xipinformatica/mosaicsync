...
test("1.30 has one exact release identity across public/runtime surfaces", () => {
  ...
  const releaseNotes = fs.readFileSync("docs/RELEASE-1.30.5.md", "utf8");
  const qa = fs.readFileSync("docs/QA-1.30.5.md", "utf8");
  ...
  assert.match(constants, /export const VERSION = "1\.30\.5";/);
  assert.match(ffHtml, /MosaicSync · 1\.30\.5/g);
  assert.match(chromeHtml, /MosaicSync · 1\.30\.5/g);
  assert.match(readme, /Current source release: 1\.30\.5/);
  assert.match(devReadme, /1\.30/);
  assert.match(releaseNotes, /^# MosaicSync 1\.30\.5 publication notes/m);
  assert.match(releaseNotes, /## GitHub release title\n\n`MosaicSync 1\.30\.5`/);
  assert.match(qa, /^# MosaicSync 1\.30\.5 QA \/ release-candidate checklist/m);
...
test("1.30 public changelog keeps internal candidates out of the public release sequence", () => {
  ...
  assert.deepEqual(headings.slice(0, 10), [VERSION, "1.30.4", "1.30.2", "1.30.1", "1.27.9", "1.27.8.9", "1.27.7", "1.27.6", "1.27.5", "1.27.4"]);
});
