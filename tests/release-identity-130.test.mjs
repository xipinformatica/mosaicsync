...
test("1.30 has one exact release identity across public/runtime surfaces", () => {
  ...
  const releaseNotes = fs.readFileSync("docs/RELEASE-1.30.md", "utf8");
  const qa = fs.readFileSync("docs/QA-1.30.md", "utf8");
  ...
  assert.match(constants, /export const VERSION = "1\.30";/);
  assert.match(ffHtml, /MosaicSync · 1\.30/g);
  assert.match(chromeHtml, /MosaicSync · 1\.30/g);
  assert.match(readme, /Current source release: 1\.30/);
  assert.match(devReadme, /1\.30/);
  assert.match(releaseNotes, /^# MosaicSync 1\.30 publication notes/m);
  assert.match(releaseNotes, /## GitHub release title\n\n`MosaicSync 1\.30`/);
  assert.match(qa, /^# MosaicSync 1\.30 QA \/ release-candidate checklist/m);
