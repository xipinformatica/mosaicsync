...
test("1.27.1 has one exact release identity across public/runtime surfaces", () => {
  ...
  assert.match(constants, /export const VERSION = "1\.27\.1";/);
  assert.match(ffHtml, /MosaicSync · 1\.27\.1/g);
  assert.match(chromeHtml, /MosaicSync · 1\.27\.1/g);
  assert.match(readme, /Current source release: 1\.27\.1/);
});

test("1.27.1 public changelog does not invent standalone unpublished releases", () => {
  const changelog = fs.readFileSync("CHANGELOG.md", "utf8");
  assert.match(changelog, /^## 1\.27\.1\n/);
  ...
  assert.deepEqual(headings.slice(0, 8), [VERSION, "1.27.0", "1.26.17.7", "1.26.17.6", "1.26.17.5", "1.26.17.4", "1.26.17.3", "1.26.12"]);
});
