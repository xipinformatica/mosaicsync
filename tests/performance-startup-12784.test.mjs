...
for (const browser of ["firefox", "chrome"]) {
  test(`1.27.8.5 ${browser} uses a genuinely secondary-only stylesheet`, () => {
    ...
    assert.ok(Buffer.byteLength(critical) + Buffer.byteLength(secondary) < 128_000,
      "runtime CSS must stay within the reviewed post-monolith budget, including the 1.31.4 localization-safe wallpaper-dim layout");
