...
const VERSION = "1.26.17.5";

test("1.26.17.5 has one exact release identity across public/runtime surfaces", () => {
  ...
  assert.match(constants, /export const VERSION = "1\.26\.17\.5";/);
  assert.match(ffHtml, /MosaicSync · 1\.26\.17\.5/);
  assert.match(chromeHtml, /MosaicSync · 1\.26\.17\.5/);
  assert.match(readme, /Current source release: 1\.26\.17\.5/);
});

test("1.26.17.5 public changelog does not invent standalone unpublished releases", () => {
  const changelog = fs.readFileSync("CHANGELOG.md", "utf8");
  assert.match(changelog, /^## 1\.26\.17\.5\n/);
  ...
  assert.deepEqual(headings.slice(0, 4), [VERSION, "1.26.17.4", "1.26.17.3", "1.26.12"]);
});
