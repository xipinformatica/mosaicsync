...
async function resolveFaviconForUrl(pageUrl, { timeoutMs = ICON_RECOVERY_FETCH_TIMEOUT_MS, preferQuality = false } = {}) {
  if (!(await hasWebAccess())) return { image: "", sourceUrl: "", reason: "permission", provisional: false };
  ...
  // Fast pass: browser-known artwork is useful immediately, but its intrinsic
  // quality is not assumed. A later quality pass may replace it.
  const native = await resolveBrowserCachedFavicon(pageUrl);
  if (native?.image) {
    best = native;
    if (!preferQuality) return { ...native, provisional: true };
  }
