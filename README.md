...
**Current source release: 1.33.0.6**
...
The active source release is **1.33.0.6** across both browser manifests, Chrome `version_name`, the shared runtime `VERSION`, the Settings version label, package filenames and current release tests. `build-manifest.json` records the same technical version for both generated browser trees.
...
1.33.0.6 is Snow Leopard II Step 3B. The Bookmarks dialog shell and its dedicated controller leave ordinary New Tab startup and are dynamically materialized on first use. Initial live DOM falls to 598 elements, eager ID bindings to 186, and the static New Tab module closure to 23 modules / 640,462 raw source bytes. The historical controller remains the owner after first use; the browser Bookmarks API remains lazy. Step 3 remains in progress. See [docs/SNOW-LEOPARD-II.md](docs/SNOW-LEOPARD-II.md).
