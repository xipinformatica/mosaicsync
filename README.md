...
**Current source release: 1.27.8.8**
...
The active source release is **1.27.8.8** across both browser manifests, Chrome `version_name`, the shared runtime `VERSION`, the Settings version label, package filenames and current release tests. `build-manifest.json` records the same technical version for both generated browser trees.
...
1.27.8.8 combines whole-profile Sync recovery with a substantial New Tab startup/performance pass. Trusted devices publish Personal and Work together as one complete, root-last safety generation while retaining the immediately previous verified generation; fresh profiles no longer become ready from Personal alone, local edits made while waiting are merged safely, and trusted complete snapshots can repair torn Work compatibility data without weakening existing record conflict/tombstone rules.
