...
**Current source release: 1.32.1.4**
...
The active source release is **1.32.1.4** across both browser manifests, Chrome `version_name`, the shared runtime `VERSION`, the Settings version label, package filenames and current release tests. `build-manifest.json` records the same technical version for both generated browser trees.
...
1.32.1.4 is a narrow normalization-correctness corrective over 1.32.1.3. Independent state-machine fuzzing found that two valid top-level records sharing one grid position could require a second normalization pass before array order matched the repaired positions. Ordinary two-device Sync can produce that exact collision when two devices concurrently move different shortcuts into the same slot. The bounded repair policy now preserves forward spatial intent, wraps only when necessary, and returns final-position order so `normalizeState(normalizeState(x))` is a fixed point after one pass. Profile export/import is canonical after the same repair. No permission, persisted-state schema, profile-format, Sync/Recovery wire-format, CSP or browser-floor changes.
