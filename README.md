...
**Current source release: 1.27.8.9**
...
The active source release is **1.27.8.9** across both browser manifests, Chrome `version_name`, the shared runtime `VERSION`, the Settings version label, package filenames and current release tests. `build-manifest.json` records the same technical version for both generated browser trees.
...
1.27.8.9 combines whole-profile Sync recovery with a substantial New Tab startup/performance pass and the final real-hardware corrective fixes from the internal 1.27.8.x candidates. Trusted devices publish Personal and Work together as one complete, root-last safety generation while retaining the immediately previous verified generation; fresh profiles no longer become ready from Personal alone, local edits made while waiting are merged safely, and trusted complete snapshots can repair torn Work compatibility data without weakening existing record conflict/tombstone rules.

The New Tab critical path now uses a smaller launcher-only stylesheet, begins authoritative local-storage I/O before the main module graph consumes it, adopts an exactly matching bootstrap grid in place, hydrates only visible closed-folder artwork first, warms hidden artwork in bounded yielding chunks, keeps startup diagnostics local/ephemeral, and synchronizes Frequently Visited Show/Count intent while keeping actual browser data and permissions device-local. Light/Dark wallpaper previews remain isolated while Settings is open, and shortcut hover uses a restrained paint-only `scale(1.045)` / `brightness(1.065)` effect with no grid reflow.

The final startup-style lifecycle removes the deliberate post-first-paint insertion of `newtab-secondary.css`. The mascot animation remains fully critical-owned; Light mode establishes its effective-theme attribute during the first-paint bootstrap; external state commits are deferred while Settings is open; drag/drop choice text is refreshed from locale keys; and favicon selection uses bounded suitability rather than unbounded resolution dominance. Secondary UI CSS is now loaded idempotently only immediately before Settings, Bookmarks, the shortcut editor, folder/drop menus, launcher context menus, toasts or the brand animation can become visible. The automatically surfaced Website Access prompt is fully critical-styled so normal startup never needs to inject the secondary stylesheet. Launcher native buttons inserted around first paint also explicitly suppress native widget appearance. No new permissions, CSP relaxation, telemetry, remote code or external service were added.
