...
**Current source release: 1.32.1.3**
...
The active source release is **1.32.1.3** across both browser manifests, Chrome `version_name`, the shared runtime `VERSION`, the Settings version label, package filenames and current release tests. `build-manifest.json` records the same technical version for both generated browser trees.
...
1.32.1.3 is a narrow post-audit correctness corrective over 1.32.1.2. Top-level placement is capacity-aware across ordinary shortcut creation, Frequently Visited insertion, cross-Space moves, folder ungrouping and grid shrink; failed full-grid operations preserve the previous authoritative state. Profile import now performs bounded structural preflight before integrity/asset traversal and applies strict imported-raster validation to PNG/JPEG/WebP payloads. Custom Branding import rollback is conditional/serialized so it cannot overwrite a newer local save, and failed Settings imports do not install rejected candidate state into live memory before durable persistence succeeds. Profile format remains v3 and Normal Sync/Recovery wire formats, permissions, CSP and browser floors are unchanged.
