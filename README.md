...
**Current source release: 1.30.18.15**
...
The active source release is **1.30.18.15** across both browser manifests, Chrome `version_name`, the shared runtime `VERSION`, the Settings version label, package filenames and current release tests. `build-manifest.json` records the same technical version for both generated browser trees.
...
1.30.18.15 begins **Step 3.1** by replacing the two ~6,800-line Firefox/Chrome background implementations with one canonical shared background core plus small platform adapters. Sync, Recovery, metadata, queues, alarms and favicon policy now have one semantic owner; Firefox/Chrome adapters retain only genuine permission/native-favicon/protected-page differences. No product feature, first-paint/cache ownership, state/meta/Sync/Recovery schema, permission or CSP change is introduced.
